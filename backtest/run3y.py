#!/usr/bin/env python3
"""Backtest 3 ans (2023-2026) — script standalone."""
...
# ── Données ───────────────────────────────────────────────────────────────────
print(f"\n{Fore.CYAN}{'='*65}")
print(f"  BACKTEST 3 ANS — {START} → {END}")
...
print(f"  RÉSULTATS — BACKTEST 3 ANS ({START} → {END}) | Capital initial : {INITIAL:.0f}€")
...
pd.DataFrame(rows).to_csv(f"{RESULTS}/run3y_summary.csv", index=False)
print(f"\nSauvegardé : {RESULTS}/run3y_summary.csv")
...
try:
    ...
    plt.tight_layout()
    out = f"{RESULTS}/run3y_equity.png"
