#!/usr/bin/env python3
"""Backtest 4 ans (2022-2026) — script standalone."""
...
START = "2021-04-29"
END = "2025-04-29"
PERIOD = "5y"  # PLTR dispo 2020-09 → période complète 21 actifs
EURUSD = 1.1608
RESULTS = "backtest/results"
os.makedirs(RESULTS, exist_ok=True)

XSTOCK_MAP = {
    # Univers Alpaca migration 2026-05-01 (tickers réels sans suffixe x)
...
def fetch(ticker, to_eur=False):
    try:
        ...
        df = df[(df.index >= START) & (df.index <= END)].dropna(subset=["close"])
        if to_eur:
...
# ── Données ───────────────────────────────────────────────────────────────────
print(f"\n{Fore.CYAN}{'='*65}")
print(f"  BACKTEST 4 ANS — {START} → {END}")
...
print(f"  RÉSULTATS — BACKTEST 4 ANS ({START} → {END}) | Capital initial : {INITIAL:.0f}€")
...
# ── Benchmarks ────────────────────────────────────────────────────────────────
print(f"\n{Fore.CYAN}── Benchmarks ({START} → {END}) ──{Style.RESET_ALL}")
for tk, label in [("^GSPC","S&P 500"), ("QQQ","NASDAQ-100"), ("BTC-EUR","BTC/EUR buy&hold")]:
    try:
        raw = yf.Ticker(tk).history(period=PERIOD, interval="1d")["Close"]
        raw.index = pd.to_datetime(raw.index)
        if raw.index.tz is not None:
            raw.index = raw.index.tz_localize(None)
        raw = raw[(raw.index >= START) & (raw.index <= END)].dropna()
...
for key in BOT_FUNCS:
    ...
    rows.append({"bot": key.upper(), "strategy": BOT_NAMES[key],
                 "period": f"{START}/{END}", "cagr_pct": m["cagr"],
...
for key, name in Z_LABELS.items():
    ...
    if z and z.get("equity") and z.get("metrics"):
        ...
        rows.append({"bot": f"Z-{key.upper()}", "strategy": name,
                     "period": f"{START}/{END}", "cagr_pct": m.get("cagr", 0),
                     "sharpe": m.get("sharpe", 0), "max_dd_pct": m.get("max_dd", 0),
                     "trades": 0, "win_rate_pct": 0,
                     "final_eur": m.get("final", 0), "profit_factor": 0})
pd.DataFrame(rows).to_csv(f"{RESULTS}/run4y_summary.csv", index=False)
print(f"\nSauvegardé : {RESULTS}/run4y_summary.csv")
...
try:
    ...
    fig.suptitle(f"Backtest 3 ans — {START} → {END}", fontsize=13, fontweight="bold")
    ...
    plt.tight_layout()
    out = f"{RESULTS}/run4y_equity.png"
