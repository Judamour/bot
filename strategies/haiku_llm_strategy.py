"""
Bot F: LLM-Driven Strategy — Claude Haiku 4.5

Même prompt que Bot E (Sonnet) et Bot D (DeepSeek R1).
Objectif : voir ce qu'un petit modèle rapide et pas cher vaut en trading.
...
STATE_FILE = "logs/haiku_llm/state.json"
INITIAL_CAPITAL = 1000.0
POSITION_SIZE = 100.0
MAX_POSITIONS = 6
CLAUDE_MODEL = os.getenv("HAIKU_LLM_MODEL", "claude-haiku-4-5-20251001")
# Pricing Claude Haiku 4.5
_INPUT_PRICE_PER_M  = 0.80  # $0.80 / 1M tokens
_OUTPUT_PRICE_PER_M = 4.0   # $4.00 / 1M tokens
...
def log(msg: str, level: str = "INFO"):
    ...
    print(f"{ts} [BOT-F][{level}] {msg}")
    os.makedirs("logs", exist_ok=True)
    with open("logs/haiku_llm.log", "a") as f:
...
def run_haiku_cycle(state: dict, ohlcv_4h: dict, macro: dict) -> dict:
    """Run one cycle of the Haiku LLM-driven strategy."""
    for symbol in config.SYMBOLS:
        ...
        decision, usage = _call_haiku(prompt)
        ...
        log(
            ...
            f"[↑{usage['input']}+{usage['output']} tok | cumul ${stats['est_cost_usd']:.4f}]"
        )
