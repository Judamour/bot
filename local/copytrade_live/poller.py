"""Main loop: ssh-poll VPS decisions.jsonl → filter surfandturf → dispatch BUY/SELL.

Lance via: python -m local.copytrade_live.poller
ou via systemd --user (copytrade-live.service).
"""
import json
import logging
import re
...
def fetch_vps_decisions(since_ts: int, tail_lines: int = 200) -> list[dict]:
    """SSH vers VPS et récupère les N dernières décisions plus récentes que since_ts."""
    cmd = ["ssh", "-o", "ConnectTimeout=10", "-o", "BatchMode=yes",
           config.VPS_HOST, f"tail -n {tail_lines} {shlex.quote(config.VPS_DECISIONS_PATH)}"]
    try:
        out = subprocess.run(cmd, capture_output=True, text=True, timeout=20)
    except subprocess.TimeoutExpired:
        log.warning("SSH timeout, skip cycle")
        return []
    if out.returncode != 0:
        log.warning(f"SSH rc={out.returncode}: {out.stderr[:200]}")
        return []
    decisions = []
...
def handle_buy(decision: dict, positions: dict) -> None:
    if len(positions) >= config.MAX_POSITIONS:
        log.info(f"SKIP BUY: max_positions={config.MAX_POSITIONS} atteint")
    ...
    if decision.get("target_size_usd", 0) < config.MIN_TARGET_SIZE_USD:
        log.info(f"SKIP BUY: target trade < ${config.MIN_TARGET_SIZE_USD}")
    ...
    if result.get("status") in ("dry_run", "submitted"):
        state.record_buy(
            ...
            size_shares=result["size_shares"],
...
def handle_sell(decision: dict, positions: dict) -> None:
    ...
    if not pos:
        log.info(f"SKIP SELL: pas de position locale sur token={token_id[:10]}...")
    ...
    if result.get("status") in ("dry_run", "submitted"):
        _, realized = state.record_sell(
            positions,
            token_id=token_id,
            size_shares=size_to_sell,
    ...
    write_jsonl("trades.jsonl", {**decision, "local_action": "sell", "fraction": fraction, "exec_result": result})


def cycle(meta: dict, positions: dict) -> None:
    decisions = fetch_vps_decisions(since_ts=meta["last_seen_ts"])
    if not decisions:
        return
    ...
    for d in sorted(relevant, key=lambda x: x["ts"]):
        try:
            ...
            log.error(f"Erreur traitement décision ts={d.get('ts')}: {type(e).__name__}: {e}")
...
def main() -> None:
    ...
    log.info(f"State: last_seen_ts={meta['last_seen_ts']}, {len(positions)} positions locales")
    ...
    try:
        ...
        if clob_bal < config.KILL_EQUITY_USD:
            log.warning(f"Solde CLOB sous kill_eq (${clob_bal:.2f} < ${config.KILL_EQUITY_USD}) — pause préventive")
    except Exception as e:
        log.warning(f"Boot: get_clob_balance échec: {e}")
