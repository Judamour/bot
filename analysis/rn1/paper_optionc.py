...
systemd : deploy/rn1-paper-optionc.service
"""
...
DATA_DIR = Path(__file__).resolve().parent / "data_optionc"
...
POLL_INTERVAL_S = int(os.environ.get("RN1_OPTIONC_POLL_S", "60"))
# RN1 splits big orders into many small ones in seconds; coalesce them.
DEDUP_WINDOW_S = int(os.environ.get("RN1_OPTIONC_DEDUP_WINDOW_S", "300"))

# --- Capital + position caps (mirror live $40 surfandturf constraints) ---
INITIAL_CAPITAL_USD = float(os.environ.get("RN1_OPTIONC_INITIAL", "40"))
MAX_POSITIONS = int(os.environ.get("RN1_OPTIONC_MAX_POSITIONS", "8"))
MAX_USD_PER_MARKET = float(os.environ.get("RN1_OPTIONC_MAX_USD_PER_MARKET", "5.0"))
MIN_TARGET_SIZE_USD = float(os.environ.get("RN1_OPTIONC_MIN_TARGET", "200"))
TAKER_FEE = float(os.environ.get("RN1_OPTIONC_TAKER_FEE", "0.02"))

# --- Tiered bands (RN1-tuned) ---
TIER_PENNY_MIN = float(os.environ.get("RN1_OPTIONC_PENNY_MIN", "0.06"))
TIER_PENNY_MAX = float(os.environ.get("RN1_OPTIONC_PENNY_MAX", "0.20"))
TIER_PENNY_SIZE = float(os.environ.get("RN1_OPTIONC_PENNY_SIZE", "1.0"))
TIER_SKIP_HIGH = float(os.environ.get("RN1_OPTIONC_SKIP_HIGH", "0.45"))  # below this AND above PENNY_MAX = skip
TIER_NORMAL_MAX = float(os.environ.get("RN1_OPTIONC_NORMAL_MAX", "0.95"))
TIER_NORMAL_SIZE = float(os.environ.get("RN1_OPTIONC_NORMAL_SIZE", "4.5"))
...
log = logging.getLogger("rn1-optionc")
...
def _trade_passes(trade: dict, market: dict | None) -> tuple[bool, str]:
    ...
    # - Hours 18 + 20-23 (all 95-100% WR, only 19 is bad)
