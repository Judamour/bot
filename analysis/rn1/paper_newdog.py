...
Reads decisions.jsonl written by bot-cp.service, filters wallet=newdog.
Paper portfolio in data_newdog/.

Run :
    python -m analysis.rn1.paper_newdog

systemd : deploy/rn1-paper-newdog.service
"""
...
DATA_DIR = Path(__file__).resolve().parent / "data_newdog"
...
POLL_INTERVAL_S = int(os.environ.get("NEWDOG_POLL_S", "60"))
# RN1 splits big orders into many small ones in seconds; coalesce them.
DEDUP_WINDOW_S = int(os.environ.get("NEWDOG_DEDUP_WINDOW_S", "300"))

# --- Capital + position caps (mirror live $40 newdog constraints) ---
INITIAL_CAPITAL_USD = float(os.environ.get("NEWDOG_INITIAL", "40"))
MAX_POSITIONS = int(os.environ.get("NEWDOG_MAX_POSITIONS", "8"))
MAX_USD_PER_MARKET = float(os.environ.get("NEWDOG_MAX_USD_PER_MARKET", "5.0"))
MIN_TARGET_SIZE_USD = float(os.environ.get("NEWDOG_MIN_TARGET", "200"))
TAKER_FEE = float(os.environ.get("NEWDOG_TAKER_FEE", "0.02"))

# --- Tiered bands (RN1-tuned) ---
TIER_PENNY_MIN = float(os.environ.get("NEWDOG_PENNY_MIN", "0.06"))
TIER_PENNY_MAX = float(os.environ.get("NEWDOG_PENNY_MAX", "0.20"))
TIER_PENNY_SIZE = float(os.environ.get("NEWDOG_PENNY_SIZE", "1.0"))
TIER_SKIP_HIGH = float(os.environ.get("NEWDOG_SKIP_HIGH", "0.45"))  # below this AND above PENNY_MAX = skip
TIER_NORMAL_MAX = float(os.environ.get("NEWDOG_NORMAL_MAX", "0.95"))
TIER_NORMAL_SIZE = float(os.environ.get("NEWDOG_NORMAL_SIZE", "4.5"))
...
def main() -> None:
    ...
    log.info(f"Boot — newdogbeginning paper (quasi-cert arbitrageur 0xfea31bc0…d227), "
