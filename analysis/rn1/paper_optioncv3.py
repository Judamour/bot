"""RN1 paper bot — Option C v3: C v2 + expanded toxic-hour skip.

Built on Option C v2 (lottery + h19 + whale). Extends the hour filter from
{19} to {0, 1, 19, 21} UTC based on 2026-05-20 hour-by-hour backtest:

  TOXIC HOURS (skip):
    - 19 UTC : EU football evening kickoff + RG Qualif live (-$10.14 / 45 trades)
    - 21 UTC : EU football mid-2nd-half live (-$5 / 3 trades, -49.87% ROI)
    - 00 UTC : MLB US prime-time live arb (-$22 / 26 trades, -18.69% ROI)
    - 01 UTC : MLB US late live arb (-$2.59 / 11 trades)

  WHY : same sport / mtype is profitable in pre-event (h14-17 UTC) and
  loses in live hours (h00-01, h18-21). RN1's edge is 45s information
  advantage on live-score feeds (Phemex Apr 2026), uncopyable via REST.
  These hours = pure live-arb dominance. Skip them = proxy for "skip live"
  without the over-restriction of Bot E v2.1's pre_event filter.

Backtest delta on 4 days:
    Bot C v2 (skip h19 only)        : +$143 / 293 trades / +11.74% ROI
    Bot C v3 (skip h0,h1,h19,h21)   : +$171 / 268 trades / +15.33% ROI
                                       Δ = +$27 / +3.6 pp ROI / 64.6% WR

Same sizing as C v2: penny $1, normal $4.5, MIN_TARGET=$200, $4.5/market.

Run :
    python -m analysis.rn1.paper_optioncv3

systemd : deploy/rn1-paper-optioncv3.service
"""
...
DATA_DIR = Path(__file__).resolve().parent / "data_optioncv3"
...
POLL_INTERVAL_S = int(os.environ.get("RN1_OPTIONCV3_POLL_S", "60"))
# RN1 splits big orders into many small ones in seconds; coalesce them.
DEDUP_WINDOW_S = int(os.environ.get("RN1_OPTIONCV3_DEDUP_WINDOW_S", "300"))

# --- Capital + position caps (mirror live $40 surfandturf constraints) ---
INITIAL_CAPITAL_USD = float(os.environ.get("RN1_OPTIONCV3_INITIAL", "600"))
MAX_POSITIONS = int(os.environ.get("RN1_OPTIONCV3_MAX_POSITIONS", "8"))
MAX_USD_PER_MARKET = float(os.environ.get("RN1_OPTIONCV3_MAX_USD_PER_MARKET", "5.0"))
MIN_TARGET_SIZE_USD = float(os.environ.get("RN1_OPTIONCV3_MIN_TARGET", "200"))
TAKER_FEE = float(os.environ.get("RN1_OPTIONCV3_TAKER_FEE", "0.02"))

# --- Tiered bands (RN1-tuned) ---
TIER_PENNY_MIN = float(os.environ.get("RN1_OPTIONCV3_PENNY_MIN", "0.06"))
TIER_PENNY_MAX = float(os.environ.get("RN1_OPTIONCV3_PENNY_MAX", "0.20"))
TIER_PENNY_SIZE = float(os.environ.get("RN1_OPTIONCV3_PENNY_SIZE", "1.0"))
TIER_SKIP_HIGH = float(os.environ.get("RN1_OPTIONCV3_SKIP_HIGH", "0.45"))  # below this AND above PENNY_MAX = skip
TIER_NORMAL_MAX = float(os.environ.get("RN1_OPTIONCV3_NORMAL_MAX", "0.95"))
TIER_NORMAL_SIZE = float(os.environ.get("RN1_OPTIONCV3_NORMAL_SIZE", "4.5"))
...
log = logging.getLogger("rn1-optioncv3")
...
def _trade_passes(trade: dict, market: dict | None) -> tuple[bool, str]:
    ...
    # --- Option C v3 filters — 2026-05-20 PM expanded toxic hours ---
    # Per-hour backtest revealed clusters of live-arb dominance:
    #   h00 UTC = MLB US prime-time live  (26 trades, -$22, -18.69% ROI)
    #   h01 UTC = MLB US late live        (11 trades, -$2.59, -5.63% ROI)
    #   h19 UTC = EU football evening live (45 trades, -$10.14, -10.14% ROI)
    #   h21 UTC = EU football late live   ( 3 trades, -$5, -49.87% ROI)
    # These are RN1's live-score arb hours — uncopyable via REST polling.

    # 1. Skip toxic live-arb hours
    ts = int(trade.get("timestamp") or trade.get("ts") or 0)
    if ts > 0:
        hour_utc = datetime.fromtimestamp(ts, tz=timezone.utc).hour
        if hour_utc in (0, 1, 19, 21):
            return False, f"optcv3_hour_toxic({hour_utc}h)"

    # 2. Skip lottery tickets (entry price < $0.06) — 36.4% WR confirmed loser
    if price < 0.06:
        return False, f"optcv3_lottery({price:.3f})"

    # 3. Skip whale trades (>$10K target_size = manipulation/desperate DCA)
    if target_size and target_size > 10000:
        return False, f"optcv3_whale({target_size:.0f})"
    # --- end Option C v3 filters ---
...
def _cycle(state: dict, positions: dict, markets: dict) -> tuple[int, int, int]:
    ...
    if n_examined or n_opened or n_resolved_total or skip_counts:
        ...
        log.info(f"cycle: OPTCV3 examined={n_examined} opened={n_opened} "
...
def main() -> None:
    ...
    log.info(f"Boot — RN1 paper Option C v3 (skip h{{0,1,19,21}}+lottery+whale), "
