"""RN1 paper bot — absolute-band sizing, raw mirror of RN1 BUY flow.
...
Effective bands with service env (SKIP_HIGH=0.20, NORMAL_MAX=0.95):
  - <0.06              -> SKIP (lottery)
  - [0.06-0.20)        -> $1.00 (penny, floored to 5 shares via MIN_SHARES)
  - [0.20-0.95]        -> $4.50 (normal — mid_low confirmed +5.54% ROI in
                                 2026-05-20 6010-trade REDEEM analysis;
                                 original "losing_zone" skip is dead code
                                 because the env collapses it to 0)
  - >0.95              -> SKIP (edge too thin)

No filters beyond MIN_TARGET — this is the baseline against which Option B
and Option C are measured.

Run :
    python -m analysis.rn1.paper_absband

systemd : deploy/rn1-paper-absband.service
"""
...
DATA_DIR = Path(__file__).resolve().parent / "data_absband"
...
POLL_INTERVAL_S = int(os.environ.get("RN1_ABSBAND_POLL_S", "60"))
# RN1 splits big orders into many small ones in seconds; coalesce them.
DEDUP_WINDOW_S = int(os.environ.get("RN1_ABSBAND_DEDUP_WINDOW_S", "300"))

# --- Capital + position caps (mirror live $40 surfandturf constraints) ---
INITIAL_CAPITAL_USD = float(os.environ.get("RN1_ABSBAND_INITIAL", "40"))
MAX_POSITIONS = int(os.environ.get("RN1_ABSBAND_MAX_POSITIONS", "8"))
MAX_USD_PER_MARKET = float(os.environ.get("RN1_ABSBAND_MAX_USD_PER_MARKET", "5.0"))
MIN_TARGET_SIZE_USD = float(os.environ.get("RN1_ABSBAND_MIN_TARGET", "200"))
TAKER_FEE = float(os.environ.get("RN1_ABSBAND_TAKER_FEE", "0.02"))

# --- Tiered bands (RN1-tuned) ---
TIER_PENNY_MIN = float(os.environ.get("RN1_ABSBAND_PENNY_MIN", "0.06"))
TIER_PENNY_MAX = float(os.environ.get("RN1_ABSBAND_PENNY_MAX", "0.20"))
TIER_PENNY_SIZE = float(os.environ.get("RN1_ABSBAND_PENNY_SIZE", "1.0"))
TIER_SKIP_HIGH = float(os.environ.get("RN1_ABSBAND_SKIP_HIGH", "0.45"))  # below this AND above PENNY_MAX = skip
TIER_NORMAL_MAX = float(os.environ.get("RN1_ABSBAND_NORMAL_MAX", "0.95"))
TIER_NORMAL_SIZE = float(os.environ.get("RN1_ABSBAND_NORMAL_SIZE", "4.5"))
...
log = logging.getLogger("rn1-absband")
