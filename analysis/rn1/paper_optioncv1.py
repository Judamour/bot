"""RN1 paper bot — Option C v1 (original, control arm).

The ORIGINAL Option C filter set from session 2026-05-20 5-day MTM analysis:
  - skip hour 19 UTC
  - skip Monday (claimed 64.7% WR — invalidated by 4-day REDEEM ablation)
  - skip lottery (price < $0.06)
  - skip mtype="other" (never triggers in practice)
  - skip whale (>$10K target_size)

Kept as the "control arm" so we can verify in-vivo that the 2026-05-20 PM
ablation findings (Monday + mtype filters were faulty) hold up over time
vs C v2 (3 filters: drops Monday + mtype) and C v3 (3 filters + extended
toxic hours {0, 1, 19, 21}).

Backtest 4-day window (2026-05-20 PM):
    C v1: 260 accepted / 61.4% WR / +$129.81 / +11.91% ROI
    C v2: 293 accepted / 61.8% WR / +$143.28 / +11.74% ROI  (+$13.47 vs v1)
    C v3: 268 accepted / 64.6% WR / +$170.97 / +15.33% ROI  (+$41.16 vs v1)

Same sizing as C v2/v3: penny $1, normal $4.5, MIN_TARGET=$200, $4.5/market.

Run :
    python -m analysis.rn1.paper_optioncv1

systemd : deploy/rn1-paper-optioncv1.service
"""
...
DATA_DIR = Path(__file__).resolve().parent / "data_optioncv1"
...
POLL_INTERVAL_S = int(os.environ.get("RN1_OPTIONCV1_POLL_S", "60"))
# RN1 splits big orders into many small ones in seconds; coalesce them.
DEDUP_WINDOW_S = int(os.environ.get("RN1_OPTIONCV1_DEDUP_WINDOW_S", "300"))

# --- Capital + position caps (mirror live $40 surfandturf constraints) ---
INITIAL_CAPITAL_USD = float(os.environ.get("RN1_OPTIONCV1_INITIAL", "600"))
MAX_POSITIONS = int(os.environ.get("RN1_OPTIONCV1_MAX_POSITIONS", "8"))
MAX_USD_PER_MARKET = float(os.environ.get("RN1_OPTIONCV1_MAX_USD_PER_MARKET", "5.0"))
MIN_TARGET_SIZE_USD = float(os.environ.get("RN1_OPTIONCV1_MIN_TARGET", "200"))
TAKER_FEE = float(os.environ.get("RN1_OPTIONCV1_TAKER_FEE", "0.02"))

# --- Tiered bands (RN1-tuned) ---
TIER_PENNY_MIN = float(os.environ.get("RN1_OPTIONCV1_PENNY_MIN", "0.06"))
TIER_PENNY_MAX = float(os.environ.get("RN1_OPTIONCV1_PENNY_MAX", "0.20"))
TIER_PENNY_SIZE = float(os.environ.get("RN1_OPTIONCV1_PENNY_SIZE", "1.0"))
TIER_SKIP_HIGH = float(os.environ.get("RN1_OPTIONCV1_SKIP_HIGH", "0.45"))  # below this AND above PENNY_MAX = skip
TIER_NORMAL_MAX = float(os.environ.get("RN1_OPTIONCV1_NORMAL_MAX", "0.95"))
TIER_NORMAL_SIZE = float(os.environ.get("RN1_OPTIONCV1_NORMAL_SIZE", "4.5"))
...
log = logging.getLogger("rn1-optioncv1")
...
def _trade_passes(trade: dict, market: dict | None) -> tuple[bool, str]:
    ...
    # --- Option C v1 (original) filters — session 2026-05-20 5-day MTM analysis ---
    # This is the ORIGINAL filter set before ablation. Kept running as the
    # "baseline / control arm" so we can verify in-vivo that the ablation
    # findings (Monday + mtype filters were faulty) hold up over time vs
    # C v2 (3 filters) and C v3 (3 filters + extended hours).

    # 1. Skip hour 19 UTC (claimed 53.3% WR; hours 20-23 are 100% WR)
    ts = int(trade.get("timestamp") or trade.get("ts") or 0)
    if ts > 0:
        dt_utc = datetime.fromtimestamp(ts, tz=timezone.utc)
        if dt_utc.hour == 19:
            return False, "optcv1_hour_19"
        # 2. Skip Mondays (5-day MTM claimed 64.7% WR vs 98-100% Sun/Tue/Wed)
        if dt_utc.weekday() == 0:
            return False, "optcv1_monday"

    # 3. Skip lottery tickets (entry price < $0.06) — 36.4% WR confirmed loser
    if price < 0.06:
        return False, f"optcv1_lottery({price:.3f})"
    ...
    if mtype == "other":
        return False, f"optcv1_bad_mtype({mtype})"

    # 5. Skip whale trades (>$10K target_size = manipulation/desperate DCA)
    if target_size and target_size > 10000:
        return False, f"optcv1_whale({target_size:.0f})"
    # --- end Option C v1 filters ---
...
def _cycle(state: dict, positions: dict, markets: dict) -> tuple[int, int, int]:
    ...
    if n_examined or n_opened or n_resolved_total or skip_counts:
        ...
        log.info(f"cycle: OPTCV1 examined={n_examined} opened={n_opened} "
...
def main() -> None:
    ...
    log.info(f"Boot — RN1 paper Option C v1 ORIGINAL (h19+monday+lottery+mtype+whale), "
