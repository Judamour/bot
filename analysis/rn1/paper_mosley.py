...
Reads decisions.jsonl written by bot-cp.service, filters wallet=mosley.
Paper portfolio in data_mosley/.

Run :
    python -m analysis.rn1.paper_mosley

systemd : deploy/rn1-paper-mosley.service
"""
...
DATA_DIR = Path(__file__).resolve().parent / "data_mosley"
...
POLL_INTERVAL_S = int(os.environ.get("MOSLEY_POLL_S", "60"))
# RN1 splits big orders into many small ones in seconds; coalesce them.
DEDUP_WINDOW_S = int(os.environ.get("MOSLEY_DEDUP_WINDOW_S", "300"))

# --- Capital + position caps (mirror live $40 mosley constraints) ---
INITIAL_CAPITAL_USD = float(os.environ.get("MOSLEY_INITIAL", "40"))
MAX_POSITIONS = int(os.environ.get("MOSLEY_MAX_POSITIONS", "8"))
MAX_USD_PER_MARKET = float(os.environ.get("MOSLEY_MAX_USD_PER_MARKET", "5.0"))
MIN_TARGET_SIZE_USD = float(os.environ.get("MOSLEY_MIN_TARGET", "200"))
TAKER_FEE = float(os.environ.get("MOSLEY_TAKER_FEE", "0.02"))

# --- Tiered bands (RN1-tuned) ---
TIER_PENNY_MIN = float(os.environ.get("MOSLEY_PENNY_MIN", "0.06"))
TIER_PENNY_MAX = float(os.environ.get("MOSLEY_PENNY_MAX", "0.20"))
TIER_PENNY_SIZE = float(os.environ.get("MOSLEY_PENNY_SIZE", "1.0"))
TIER_SKIP_HIGH = float(os.environ.get("MOSLEY_SKIP_HIGH", "0.45"))  # below this AND above PENNY_MAX = skip
TIER_NORMAL_MAX = float(os.environ.get("MOSLEY_NORMAL_MAX", "0.95"))
TIER_NORMAL_SIZE = float(os.environ.get("MOSLEY_NORMAL_SIZE", "4.5"))
...
def _cycle(state: dict, positions: dict, markets: dict) -> tuple[int, int, int]:
    ...
    if n_examined or n_opened or n_resolved_total or skip_counts:
        ...
        log.info(f"cycle: MOSLEY examined={n_examined} opened={n_opened} "
...
def main() -> None:
    ...
    log.info(f"Boot — Mosley1 paper (whale conviction NBA/foot 0x5bec79df…b215), "
